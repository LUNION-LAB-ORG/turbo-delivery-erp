...
const Delete = ({ fraisDeLivraisonId, open, setOpen }: { fraisDeLivraisonId: string| undefined; open: boolean; setOpen: (open: boolean) => void }) => {
    const { pending } = useFormStatus();
    const router = useRouter();
    const handleSubmit = async () => {
        if(fraisDeLivraisonId){
        const result = await deletePriceList(fraisDeLivraisonId);
                        ...
                        >
                            <DialogPanel className="panel w-full max-w-lg overflow-hidden rounded-lg border-0 p-0 text-black dark:text-white-dark">
                                ...
                                <div className="grid gap-4 p-5">
                                    <p className="text-gray-500 dark:text-gray-400">Voulez-vous supprimer cet cette livraison ?</p>
