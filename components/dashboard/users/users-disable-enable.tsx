...
const UsersDisableEnable = ({ user, open, setOpen }: { user: User; open: boolean; setOpen: (open: boolean) => void }) => {
                        ...
                        >
                            <DialogPanel className="panel w-full max-w-lg overflow-hidden rounded-lg border-0 p-0 text-black dark:text-white-dark">
                                ...
                                <div className="bg-[#fbfbfb] py-3 text-lg font-medium ltr:pl-5 ltr:pr-[50px] rtl:pl-[50px] rtl:pr-5 dark:bg-[#121c2c] text-primary">
                                    {user.status === 1 ? 'Désactiver' : 'Activer'} l&apos; utilisateur
                                </div>
                                <div className="grid gap-4 p-5">
                                    <p className="text-gray-500 dark:text-gray-400">{user.status === 1 ? 'Voulez-vous désactiver cet utilisateur ?' : 'Voulez-vous activer cet utilisateur ?'}</p>
