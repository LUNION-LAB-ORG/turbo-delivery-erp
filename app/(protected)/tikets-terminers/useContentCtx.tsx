...
export default function useContentCtx({ initialData, restaurants }: Props) {
    ...
    const handleCangeRestaurant = (restaurantId: any) => {
        ...
        if (restaurantId) {
            const result = { ...initialData, content: dataFilter }
            mapData(result)
    ...
    const mapData = (result: PaginatedResponse<BonLivraison> | null) => {
        const data = result?.content.filter((item: BonLivraison) => item.statut === "TERMINER" as any);
